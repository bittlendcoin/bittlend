...
#if defined(NDEBUG)
# error "Bittlendcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8a0f34fb7a28009f5822ae66fd58959e9c862c1c70a3384624464287c91a75e9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bittlendcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bittlendcoin Signed Message:\n";
...
{
    // Bittlendcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bittlendcoin
static const int64 nTargetSpacing = 1 * 60; // Bittlendcoin
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bittlendcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bittlendcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8a0f34fb7a28009f5822ae66fd58959e9c862c1c70a3384624464287c91a75e9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "31Jan2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042684610fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517400488;
        ...
        assert(block.hashMerkleRoot == uint256("0x31874a3e63bbab3bd115689bf8ca0efee934b2e33e57005b214ffc708becfab1"));
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc3, 0xd6, 0xdb }; // Bittlendcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BittlendcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BittlendcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BittlendcoinMiner(CWallet *pwallet)
{
    printf("BittlendcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bittlendcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BittlendcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BittlendcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BittlendcoinMiner, pwallet));
}
